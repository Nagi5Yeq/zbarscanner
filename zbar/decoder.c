...
#if ENABLE_DATABAR == 1
    if (dcode->databar.segs) free(dcode->databar.segs);
#endif
    if (dcode->buf) free(dcode->buf);
    free(dcode);
}

void zbar_decoder_reset(zbar_decoder_t* dcode) {
    memset(dcode, 0, (char*) &dcode->buf_alloc - (char*) dcode);
#if ENABLE_EAN == 1
    ean_reset(&dcode->ean);
#endif
#if ENABLE_I25 == 1
...
const char* _zbar_decoder_buf_dump(unsigned char* buf, unsigned int buflen) {
    ...
    if (!decoder_dump || dumplen > decoder_dumplen) {
        if (decoder_dump) free(decoder_dump);
