...
static int rs_quadratic_solve(const rs_gf256* _gf, unsigned _b, unsigned _c,
    ...
    g2 = rs_hgmul(
        _gf, rs_hgmul(_gf, c0, 255 - 2 * (255 / 15)) ^ rs_gmul(_gf, c0, c0),
        255 - 255 / 15);
    z2 = rs_gdiv(_gf, g2,
                 _gf->exp[255 - (255 / 15) * 4] ^ _gf->exp[255 - (255 / 15)]);
    l2 = rs_hgmul(
        _gf, rs_gmul(_gf, z2, z2) ^ rs_hgmul(_gf, z2, 255 - (255 / 15)) ^ c0,
        2 * (255 / 15));
...
static int rs_cubic_solve(const rs_gf256* _gf, unsigned _a, unsigned _b,
    ...
    if (nroots < 1) {
        /*The Reed-Solomon code is only valid if we can find 3 distinct roots in
           GF(2**8), so if we know there's only one, we don't actually need to
          find it. Note that we're also called by the quartic solver, but if we
          contain a non-trivial irreducible factor, than so does the original
           quartic~\cite{LW72}, and failing to return a root here actually saves
          us some work there, also.*/
        return 0;
    }
    /*Recover w from z.*/
    logw = _gf->log[_x[0]];
    if (logw) {
        ...
        _x[0] = _a;
        /*In this case _x[1] is a double root, so we know the Reed-Solomon code
          is invalid. Note that we still have to return at least one root,
          because if we're being called by the quartic solver, the quartic might
          still have 4 distinct roots. But we don't need more than one root, so
          we can avoid computing the expensive one.*/
...
static int rs_quartic_solve(const rs_gf256* _gf, unsigned _a, unsigned _b,
    ...
    if (_a) {
        ...
        if (t) {
            ...
            /*s must be a root~\cite{LW72}, and is in fact a
              double-root~\cite{CCO69}. Thus we're left with only a quadratic to
              solve.
              @ARTICLE{CCO69,
            ...
            /*s may be a triple root if s=_b/_a, but not quadruple, since
             * _a!=0.*/
    ...
    if (nroots < 1) {
        /*The Reed-Solomon code is only valid if we can find 4 distinct roots in
           GF(2**8).
          If the cubic does not factor into 3 (possibly duplicate) roots, then
          we know that the quartic must have a non-trivial irreducible factor.*/
        return 0;
    }
    r = _x[0];
...
static int rs_find_roots(const rs_gf256* _gf, unsigned char* _epos,
    ...
    if (_nerrors <= 4) {
        /*Explicit solutions for higher degrees are possible.
          Chien uses large lookup tables to solve quintics, and Truong et al.
          give special algorithms for degree up through 11, though they use
          exhaustive search (with reduced complexity) for some portions.
          Quartics are good enough for reading CDs, and represent a reasonable
          code complexity trade-off without requiring any extra tables. Note
          that _lambda[0] is always 1.*/
    ...
    for (i = 0; i < _npar; i++)
        if (s[i]) {
            ...
            nerrors = rs_modified_berlekamp_massey(
                _gf, lambda, s, omega, _npar, _erasures, _nerasures, _ndata);
            /*If we can't locate any errors, we can't force the syndrome values
              to zero, and must have a decoding error. Conversely, if we have
              too many errors, there's no reason to even attempt the root
              search.*/
            if (nerrors <= 0 || nerrors - _nerasures > _npar - _nerasures >> 1)
                return -1;
            /*Compute the locations of the errors.
              If they are not all distinct, or some of them were outside the
              valid range for our block size, we have a decoding error.*/
...
int main(void) {
    ...
    for (k = 0; k < 64 * 1024; k++) {
        ...
        if (npar > 0) {
            ...
            nerrors = rand() % (npar + 1);
            if (nerrors > 0) {
                /*This test is not quite correct: there could be so many errors
                  it comes within (npar>>1) errors of another valid codeword. I
                  don't know a simple way to test for that without trying to
                  decode the corrupt codeword, though, which is the very code
            ...
            for (c = 0; c < 256; c++)
                for (d = 0; d < 256; d++) {
                    ...
                    for (i = 0; i < nroots; i++) {
                        ...
                        if (e[0]) {
                            printf("Invalid root: (0x%02X)**4 ^ "
                                   "0x%02X*(0x%02X)**3 ^ "
                                   "0x%02X*(0x%02X)**2 ^ 0x%02X(0x%02X) ^ "
                                   "0x%02X = 0x%02X\n",
                                   x[i], a, x[i], b, x[i], c, x[i], d, e[0]);
                        }
                        for (j = 0; j < i; j++)
                            if (x[i] == x[j]) {
                                printf("Repeated root %i=%i: (0x%02X)**4 ^ "
                                       "0x%02X*(0x%02X)**3 ^ "
                                       "0x%02X*(0x%02X)**2 ^ 0x%02X(0x%02X) ^ "
                                       "0x%02X = 0x%02X\n",
                    ...
                    /*We only care about missing roots if the quartic had 4
                    ...
                    if (mroots == 4)
                        for (j = 0; j < mroots; j++) {
                            for (i = 0; i < nroots; i++)
                                if (x[i] == r[j]) break;
                            if (i >= nroots) {
                                printf("Missing root: (0x%02X)**4 ^ "
                                       "0x%02X*(0x%02X)**3 ^ "
                                       "0x%02X*(0x%02X)**2 ^ 0x%02X(0x%02X) ^ "
                                       "0x%02X = 0x%02X\n",
                                       r[j], a, r[j], b, r[j], c, r[j], d,
                                       e[j]);
                            }
                        }
                }
