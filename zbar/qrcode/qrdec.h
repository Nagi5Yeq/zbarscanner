...
typedef enum qr_mode {
    ...
    /*Raw 8-bit bytes.*/
    QR_MODE_BYTE,
    /*FNC1 marker (for more info, see
      http://www.mecsw.com/specs/uccean128.html). In the "first position" data
      is formatted in accordance with GS1 General Specifications.*/
    QR_MODE_FNC1_1ST,
    /*Mode 6 reserved?*/
    ...
    QR_MODE_KANJI,
    /*FNC1 marker (for more info, see
      http://www.mecsw.com/specs/uccean128.html). In the "second position" data
      is formatted in accordance with an industry application as specified by
      AIM Inc.*/
    QR_MODE_FNC1_2ND
} qr_mode;
...
struct qr_code_data {
    ...
    unsigned char sa_index;
    /*The size of the structured-append group, or 0 if there was no S-A
     * header.*/
    unsigned char sa_size;
