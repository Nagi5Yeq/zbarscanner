...
static int text_is_latin1(const unsigned char* _text, int _len) {
    int i;
    for (i = 0; i < _len; i++) {
        /*The following line fails to compile correctly with gcc 3.4.4 on ARM
           with any optimizations enabled.*/
    ...
    for (i = 0; i < nqrdata; i++)
        if (!mark[i]) {
            ...
            if (qrdata[i].sa_size) {
                ...
                for (j = i; j < nqrdata; j++)
                    if (!mark[j]) {
                        /*TODO: We could also match version, ECC level, etc. if
                           size and parity alone are too ambiguous.*/
            ...
            fnc1_2ai = 0;
            has_kanji = 0;
            /*Step 1: Detect FNC1 markers and estimate the required buffer
             * size.*/
            for (j = 0; j < sa_size; j++)
                if (sa[j] >= 0) {
                    qrdataj = qrdata + sa[j];
                    for (k = 0; k < qrdataj->nentries; k++) {
                        ...
                        /*We assume at most 4 UTF-8 bytes per input byte.
                          I believe this is true for all the encodings we
                          actually use.*/
                        case QR_MODE_KANJI:
                            has_kanji = 1;
                        case QR_MODE_BYTE:
            ...
            sa_ntext = 0;
            /*Add the encoded Application Indicator for FNC1 in the second
             * position.*/
            if (fnc1 == MOD(ZBAR_MOD_AIM)) {
                ...
                /*The Application Indicator is a single letter.
                  We already checked that it lies in one of the ranges A...Z,
                  a...z when we decoded it.*/
                else
            ...
            char* bytebuf_text =
                (char*) malloc((sa_ctext + 1) * sizeof(*sa_text));
                ...
                for (k = 0; k <= qrdataj->nentries && !err; k++) {
                    ...
                    if (bytebuf_ntext > 0) {
                        ...
                        if (entry == NULL || (entry->mode != QR_MODE_BYTE &&
                            ...
                            /*If we have no specified encoding, attempt to
                              auto-detect it unless configured with
                              ZBAR_CFG_BINARY.*/
                            if (eci < 0) {
                                if (raw_binary) {
                                    /* copy all remaining bytes to output
                                     * buffer. */
                                    ...
                                    int ei;
                                    /*If there was data encoded in kanji mode,
                                     * assume it's SJIS.*/
                                    if (has_kanji)
                                        enc_list_mtf(enc_list, sjis_cd);
                                    /*Otherwise check for the UTF-8 BOM.
                                      UTF-8 is rarely specified with ECI, and
                                      few decoders currently support doing so,
                                      so this is the best way for encoders to
                                      reliably indicate it.*/
                                    else if (inleft >= 3 &&
                                             in[0] == (char) 0xEF &&
                                             in[1] == (char) 0xBB &&
                                             in[2] == (char) 0xBF) {
                                        in += 3;
                                        inleft -= 3;
                                        /*Actually try converting (to check
                                         * validity).*/
                                    ...
                                    /*If the text is 8-bit clean, prefer UTF-8
                                       over SJIS, since SJIS will corrupt the
                                       backslashes used for DoCoMo formats.*/
                                    ...
                                    for (ei = 0; ei < 4; ei++)
                                        if (enc_list[ei] != (iconv_t) -1) {
                                            /*According to the 2005 version of
                                              the standard, ISO/IEC 8859-1 (one
                                              hyphen) is supposed to be used,
                                              but reality is not always so (and
                                              in the 2000 version of the
                                               standard, it was JIS8/SJIS that
                                              was the default). It's got an
                                              invalid range that is used often
                                              with SJIS and UTF-8, though, which
                                              makes detection easier. However,
                                              iconv() does not properly reject
                                              characters in those ranges, since
                                              ISO-8859-1 (two hyphens) defines a
                                               number of seldom-used control
                                              code characters there. So if we
                                              see any of those characters, move
                                              this
                                               conversion to the end of the
                                              list.*/
                                            if (ei < 3 &&
                            ...
                            /*We were actually given a character set; use it.
                              The spec says that in this case, data should be
                              treated as if it came from the given character set
                              even when encoded in kanji mode.*/
                            else {
                    ...
                    case QR_MODE_ECI: {
                        ...
                        if (cur_eci <= QR_ECI_ISO8859_16 && cur_eci != 14) {
                            ...
                            /*Note that CP437 requires an iconv compiled with
                               --enable-extra-encodings, and thus may not be
                               available.*/
                            else
                                enc = "CP437";
                            ...
                            enc = "UTF-8";
                        /*Don't know what this ECI code specifies, but not an
                           encoding that we recognize.*/
                        else
                            continue;
                        eci = cur_eci;
            ...
            if (!err) {
                ...
                if (sa_size == 1)
                    ...
                    for (; syms; syms = syms->next) {
                        ...
                        next =
                            (syms->next) ? syms->next->datalen : (int) sa_ntext;
